...
INPUT_XLSX = ROOT / "tmp" / "curacion_zonas_pendientes_2026-06-27_feedback_iter2.xlsx"
OUTPUT_XLSX = ROOT / "tmp" / "curacion_zonas_pendientes_2026-06-27_feedback_iter3.xlsx"
BACKUP_PATH = ROOT / "tmp" / "db.sqlite3.backup_curacion_feedback_20260627_iter3_oscar"
DB_PATH = ROOT / "db.sqlite3"
SCRIPT_TAG = "curacion_feedback_20260627_iter3_oscar"
...
REJECTED_REMOVE_IDS = {5782, 5822}
CONFIRM_REMOVE_IDS = {5821, 5838}
PROPAGATE_IDS = {
    5785,
    5788,
    5796,
...
EXISTING_COORDINATE_IDS = {5818, 5839, 5840, 5841, 5842, 5843, 5844, 6124, 6125}
NO_ADDRESS_REPARSE_IDS = {5830, 5831, 5832, 5833, 5834, 5835, 5836, 5837}
INTERSECTION_IDS = {5784, 5817, 5818}
SAMPLE_IDS = sorted(
    {
        5749,
        5752,
...
DIRECT_FIXES = {
    5749: {"address": "Gral. Toribio de Luzuriaga 2100", "locality": "Hurlingham", "geocode": True},
    5752: {"address": "Manuel A. Padilla 1200", "locality": "Villa Tesei", "geocode": True},
    5778: {"address": "Pueyrredon 2036", "locality": "Hurlingham", "geocode": True},
    5779: {"address": "Uspallata 1931", "locality": "Hurlingham", "geocode": True},
    5780: {"address": "Plumerillos 1957", "locality": "Hurlingham", "geocode": True},
    5781: {"address": "Pablo Pizzurno 686", "locality": "Hurlingham", "geocode": True},
    5783: {"address": "Tte. Gral. Pablo Ricchieri 1259", "locality": "Hurlingham", "geocode": True},
    5784: {"address": "Jorge Daniel Thevenin y John F. Kennedy", "locality": "Hurlingham", "geocode": True},
    5786: {"address": "Santa Ana 230", "locality": "Hurlingham", "geocode": True},
    5787: {"address": "Juan de Garay 1951", "locality": "Hurlingham", "geocode": True},
    5789: {"address": "Epecuen 351", "locality": "Hurlingham", "geocode": True},
    5790: {"address": "Aconcagua 2000", "locality": "Hurlingham", "geocode": True},
    5791: {"address": "Quintino Bocayuva 218", "locality": "Hurlingham", "geocode": True},
    5792: {"address": "Plumerillos 1895", "locality": "Hurlingham", "geocode": True},
    5793: {"address": "Cjal. Enrique Recagno 700", "locality": "Hurlingham", "geocode": True},
    5794: {"address": "Canada de Gomez 544", "locality": "Hurlingham", "geocode": True},
    5795: {"address": "Tte. Gral. Julio Argentino Roca 2100", "locality": "Hurlingham", "geocode": True},
    5798: {"address": "Granaderos 334", "locality": "Hurlingham", "geocode": True},
    5799: {"address": "Granaderos 289", "locality": "Hurlingham", "geocode": True},
}
...
def residual_reason(property_obj):
    ...
    if not best_address(property_obj) or is_bad_oscar_address(property_obj.address) or is_bad_oscar_address(property_obj.detected_address):
...
def apply_geocode(property_obj, reason, summary, apply):
    ...
    if current and not current.outside_target:
        ...
        return
    if is_oscar_related(property_obj):
        listing = first_listing(property_obj)
        if listing and listing.url:
            try:
                status_code, markup = fetch_url_text(listing.url, "oscar-dahbar")
            except Exception as exc:
                add_summary(summary, "source_fetch_errors", {"id": property_obj.pk, "source": "oscar-dahbar", "error": repr(exc)})
            else:
                if status_code < 400:
                    ...
                    if coordinates:
                        ...
                        if update_location(
                            ...
                            add_summary(summary, "sweep_map_locations", {"id": property_obj.pk, "source": "oscar-dahbar", "method": coordinate.get("method")})
...
def apply_existing_coordinate_zones(summary, apply):
    for property_id in sorted(EXISTING_COORDINATE_IDS):
        property_obj = Property.objects.select_related("location", "location_intelligence").filter(pk=property_id).first()
        if not property_obj:
            add_summary(summary, "missing_property", property_id)
            continue
        current = location_or_none(property_obj)
        if not current:
            add_summary(summary, "existing_coordinate_skipped", {"id": property_id, "reason": "sin_coordenadas"})
            continue
        if current.outside_target:
            add_summary(summary, "existing_coordinate_skipped", {"id": property_id, "reason": "coordenada_fuera_target"})
            continue
        add_summary(
            summary,
            "existing_coordinate_zones",
            {"id": property_id, "lat": current.latitude, "lon": current.longitude, "provider": current.provider},
        )
        apply_zone_from_point(
            property_obj,
            current.latitude,
            current.longitude,
            current.provider,
            "Oscar/Argenprop con coordenada existente sin zona",
            summary,
            apply,
        )
...
def apply_fincas_prefix_repairs(summary, apply):
    now = timezone.now()
    candidates = list(fincas_prefix_queryset())
...
def generate_excel(reviewed_notes):
    ...
    table = Table(displayName="PendientesFeedback20260627Iter3Oscar", ref=ws.dimensions)
...
def apply_feedback(apply):
    ...
    apply_direct_fixes(summary, apply, summary["approved_found"])
    apply_oscar_propagated_addresses(summary, apply)
    apply_existing_coordinate_zones(summary, apply)
    apply_oscar_no_address_reparse(summary, apply)

    for property_id in sorted(summary["rejected_found"]):
        property_obj = Property.objects.prefetch_related("listings__source").filter(pk=property_id).first()
        if property_obj:
            mark_removed(property_obj, notes.get(property_id, "rechazado por feedback de curacion"), summary, apply)

    confirm_oscar_remove_candidates(summary, apply)
