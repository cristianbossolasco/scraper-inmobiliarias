...
INPUT_XLSX = ROOT / "tmp" / "curacion_zonas_pendientes_2026-06-27_feedback_iter3.xlsx"
OUTPUT_XLSX = ROOT / "tmp" / "curacion_zonas_pendientes_2026-06-27_feedback_iter4.xlsx"
BACKUP_PATH = ROOT / "tmp" / "db.sqlite3.backup_curacion_feedback_20260627_iter4"
DB_PATH = ROOT / "db.sqlite3"
SCRIPT_TAG = "curacion_feedback_20260627_iter4"
...
REJECTED_REMOVE_IDS = {2952, 2999, 3046, 3066, 3346, 3616, 5107, 5129, 5801, 5813}
NO_ACTION_IDS = {500, 961, 3316, 3356, 5854}
MAP_ONLY_IDS = {5327}
CONFIRM_REMOVE_IDS = set()
PROPAGATE_IDS = set()
EXISTING_COORDINATE_IDS = set()
NO_ADDRESS_REPARSE_IDS = set()
INTERSECTION_IDS = {6064}
SAMPLE_IDS = sorted(
    {
        500,
        961,
...
DIRECT_FIXES = {
    3087: {"address": "German Argerich 1300", "locality": "Hurlingham", "geocode": True},
    3181: {"address": "Virriato Unia 2412", "locality": "Villa Tesei", "geocode": True},
    3543: {"address": "Isabel del Maestro 3500", "locality": "William C. Morris", "geocode": True},
    3581: {
        "address": "Conscripto Bernardi 587",
        "locality": "Hurlingham",
        "property_type": Property.Type.OTHER,
        "property_type_evidence": "La fuente declara Tipo Galpon; se usa other porque el modelo no tiene tipo galpon.",
        "geocode": True,
    },
    5106: {"address": "Coraceros 2400", "locality": "Hurlingham", "geocode": True},
    5383: {"address": "Tte. Gral. Julio Argentino Roca 1099", "locality": "Hurlingham", "geocode": True},
    5700: {"address": "Acassuso 1925", "locality": "Hurlingham", "geocode": True},
    5739: {"address": "Tte. Manuel Origone 287", "locality": "Villa Tesei", "geocode": True},
    6064: {"address": "Jose de Malarredo e/ Paganini y Handys", "locality": "Hurlingham", "geocode": True},
    6127: {"address": "Coraceros 3115", "locality": "Hurlingham", "geocode": True},
}
...
def source_confirms_zonaprop_retired(status_code, markup):
    return status_code in {404, 410} or bool(ZONAPROP_RETIRED_RE.search(markup or ""))
...
def prioritized_source_candidates(source_slug, priority_ids=None):
    ...
    active_candidates = [prop for prop in candidates if prop.pk not in NO_ACTION_IDS]
...
def sweep_zonaprop(summary, apply):
    ...
    for property_obj in selected:
        ...
        data = coordinate.get("data") or {}
        if data.get("address") and "address" not in (property_obj.manual_overrides or {}):
            candidate = safe(data["address"])
            if candidate and candidate != property_obj.address:
                now = timezone.now()
                update_fields = set()
                mark_manual_property_field(property_obj, "address", candidate, now, update_fields, apply)
                if data.get("locality"):
                    mark_manual_property_field(property_obj, "locality", data["locality"], now, update_fields, apply)
                save_property(property_obj, update_fields, apply)
                add_summary(summary, "zonaprop_source_address_updates", {"id": property_obj.pk, "to": candidate})
...
def generate_excel(reviewed_notes):
    ...
    table = Table(displayName="PendientesFeedback20260627Iter4", ref=ws.dimensions)
...
def apply_feedback(apply):
    ...
    apply_map_only_feedback(summary, apply, summary["approved_found"])
    ...
    apply_fincas_prefix_repairs(summary, apply)
    apply_coraceros_alias_similars(summary, apply)
    sweep_becerra(summary, apply)
    sweep_zonaprop(summary, apply)
    sweep_faella(summary, apply)
