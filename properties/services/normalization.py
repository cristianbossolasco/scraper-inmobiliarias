...
STREET_ALIAS_PATTERNS = (
    (r"\b(?:(?:Av\.?|Avenida)\s*)?Vicente\s+Camargo\b", "Vicente Camargo"),
    (r"(?<!German\s)\bArgerich\b", "German Argerich"),
    ...
    (r"\b(?:General|Gral\.?)\s+Toribio\s+Luzuriaga\b", "Gral. Toribio de Luzuriaga"),
    (r"\bCnel\.?\s+M\.?\s*A\.?\s+Padilla\b", "Manuel A. Padilla"),
    (r"\bVirriato\s+Un(?:i|í)a\b", "Virriato Unia"),
    (r"\b(Dolores\s+de\s+Huici\s+\d{2,5})\s+Esq\.?.*$", r"\1"),
    (r"\b(?:Dr\.?\s*)?Delfor\s+D(?:i|í)az\b", "Dr. Delfor Díaz"),
    (r"\bDell\s+Eva\b", "Basilio Delleva"),
    (r"\b(?:Gral\.?|General)\s+Francisco\s+Miranda\b", "Gral. Francisco Miranda"),
    (r"(?<!Francisco\s)\bMiranda\b", "Gral. Francisco Miranda"),
    (r"\bMinoguyen\b", "José de Minoguye"),
    ...
    (r"\bRosas?\s+Castilllo\b", "Av. Rosas Castillo"),
    (r"\bRosa\s+Castillo\b", "Av. Rosas Castillo"),
    (r"\bBomberos\s+Celiz\b", "Bombero Celiz"),
    (r"\bGutemberg\b", "Gutenberg"),
    (r"(?<!Pablo\s)\bPizzurno\b", "Pablo Pizzurno"),
    (r"(?<!Enrique\s)\bRecagno\b", "Cjal. Enrique Recagno"),
    (r"\b(?:Av\.?|Avenida)\s+Roca\b", "Tte. Gral. Julio Argentino Roca"),
    (r"(?<!Quintino\s)\bBocayu(?:v|b)a\b", "Quintino Bocayuva"),
    (r"\bThevening\b", "Jorge Daniel Thevenin"),
    (r"\bTte\.?\s+Origone\b", "Tte. Manuel Origone"),
    (r"(?<!Manuel\s)\bOrigone\b", "Tte. Manuel Origone"),
    ...
    (r"\bDiego\s+de\s+Carabajal\b", "Diego de Carvajal"),
    (r"\bDiego\s+Carabajal\b", "Diego de Carvajal"),
    (r"(?<!Diego de )(?<!Diego )\bCarabajal\b", "Diego de Carvajal"),
    (r"\bCaroceros?\b", "Coraceros"),
    (r"\bCoracero\b", "Coraceros"),
    (r"\b(?:Vasco\s+N(?:u|ú)ñez\s+de\s+)?Balboa\b", "Vasco Núñez de Balboa"),
    (r"\b(?:Gral\.?|General)?\s*Mart(?:i|í)n\s+G(?:u|ü)emes\b", "Gral. Martín Güemes"),
    (r"(?<!Martín\s)(?<!Martin\s)\bG(?:u|ü)emes\b", "Gral. Martín Güemes"),
    (r"\bAv\.?\s*Julio\s+A\.?\s+Roca\b", "Tte. Gral. Julio Argentino Roca"),
    (r"\b(?:Teniente|Tte\.?)\s+(?:General|Gral\.?)\s+Julio\s+A\.?\s+Roca\b", "Tte. Gral. Julio Argentino Roca"),
    (r"\b(?:Teniente|Tte\.?)\s+(?:General|Gral\.?)\s+Julio\s+Argentino\s+Roca\b", "Tte. Gral. Julio Argentino Roca"),
    (r"\b(?:Pablo\s+)?Richieri\b", "Tte. Gral. Pablo Ricchieri"),
    (r"(?<!Pablo\s)\bRicchieri\b", "Tte. Gral. Pablo Ricchieri"),
    (r"\bNilda\s+Figueiras\b", "Nilda Figueira"),
    (r"\bConscripto\s+Bernardi\b", "Conscripto Bernardi"),
    (r"(?<!Conscripto\s)\bBernardi\b", "Conscripto Bernardi"),
    (r"(?<!Jos(?:é|e)\s)\bGaribaldi\b", "José Garibaldi"),
    (r"\b(?:Av\.?|Avenida)\s*(?:Gdor\.?|Gobernador)\s+Vergara\b", "Av. Gdor. Vergara"),
    (r"\b(?:Av\.?|Avenida)\s+Vergara\b", "Av. Gdor. Vergara"),
    (r"(?<!Av\. )(?<!Avenida )\b(?:Gdor\.?|Gobernador)\s+Vergara\b", "Av. Gdor. Vergara"),
    (r"\bJ\.?\s+Bustamante\s+y\s+Guevara\s+(\d{2,5})\b", r"Eva Perón \1 esquina Guevara"),
    ...
    (r"\bMaestra\s+Argentino\b", "El Maestro Argentino"),
    (r"(?<!El\s)\bMaestro\s+Argentino\b", "El Maestro Argentino"),
    (r"\bGral\.?\s+Simon\s+Bolivar\b", "Gral. Simón Bolívar"),
    (r"\bGral\.?\s+Simón\s+Bolívar\b", "Gral. Simón Bolívar"),
    (r"\bGeneral\s+Simon\s+Bolivar\b", "Gral. Simón Bolívar"),
    (r"\bGeneral\s+Simón\s+Bolívar\b", "Gral. Simón Bolívar"),
    (r"(?<!Gral\. )(?<!Gral )(?<!General )\bSim(?:ó|o)n\s+Bol(?:í|i)var\b", "Gral. Simón Bolívar"),
    (r"\bDiputado\s+Finochietto\b", "Dip. Hector Finochietto"),
